	...
	function (toClear, board) {
		...
		var newArr = A2(
			...
			A2(
				...
				function (_v0) {
					var r = _v0.a;
					return !A2($elm$core$List$member, r, toClear);
				},
				A2(
					...
					F2(
						function (i, m) {
							return _Utils_Tuple2(
								A2($author$project$Board$fromArrIndex, board, i).a,
								m);
						}),
					board.arr)));
		return _Utils_update(
			board,
			{
				arr: A2($elm$core$Array$append, newArr, newRows)
			});
	});
...
var $author$project$Tetromino$rotateMino = F2(
...
var $author$project$Tetromino$minosPositions = function (tetromino) {
	var o = tetromino.orientation;
	var rotatedMinos = A2(
		$elm$core$List$map,
		$author$project$Tetromino$rotateMino(o),
		tetromino.localPos);
	var _v0 = tetromino.pos;
	var px = _v0.a;
	var py = _v0.b;
	var shiftedMinos = A2(
		$elm$core$List$map,
...
var $author$project$Board$toArrIndex = F2(
	function (board, _v0) {
		var r = _v0.a;
		var c = _v0.b;
		return (r * board.cols) + c;
	});
var $author$project$Board$placeTetromino = F2(
	function (tetromino, board) {
		var rowsToClear = F2(
			function (i, arr) {
				rowsToClear:
				while (true) {
					if (_Utils_eq(i, board.rows)) {
						return _List_Nil;
					} else {
						var currentRow = A3($elm$core$Array$slice, i * board.cols, (i + 1) * board.cols, arr);
						var filteredRow = A2(
							$elm$core$Array$filter,
							function (m) {
						...
						if (!$elm$core$Array$length(filteredRow)) {
							return A2(
								$elm$core$List$cons,
								i,
								A2(rowsToClear, i + 1, arr));
						} else {
							var $temp$i = i + 1,
								$temp$arr = arr;
							i = $temp$i;
							arr = $temp$arr;
							continue rowsToClear;
						}
					}
				}
			});
		var folder = F2(
			function (p, arr) {
				return A3(
					$elm$core$Array$set,
					A2($author$project$Board$toArrIndex, board, p),
					tetromino.piece,
					arr);
			});
		var newArr = A3(
		...
		return A2(
			$author$project$Board$clearRows,
			A2(rowsToClear, 0, newArr),
			_Utils_update(
				board,
				{arr: newArr}));
	});
...
var $author$project$Board$inBounds = F2(
	function (board, _v0) {
		var r = _v0.a;
		var c = _v0.b;
...
var $author$project$Board$canPlace = F2(
	function (board, tetromino) {
		var minos = $author$project$Tetromino$minosPositions(tetromino);
		return A2(
			$elm$core$List$all,
			function (mino) {
...
var $author$project$GameState$doHardDrop = function (model) {
...
var $author$project$GameState$doSoftDrop = function (state) {
	return _Utils_Tuple2(
		_Utils_update(
			state,
			{
				gravityFrames: (!state.softDropping) ? 0 : state.gravityFrames,
				softDropping: true
			}),
		$elm$core$Platform$Cmd$none);
};
var $author$project$GameState$movePiece = F2(
	function (delta, state) {
		var newState = function () {
			...
			if (_v0.$ === 'Nothing') {
				...
				var t = _v0.a;
				var _v1 = A3($author$project$SRS$tryMove, state.board, t, delta);
				if (_v1.$ === 'Nothing') {
					return state;
				} else {
					var newT = _v1.a;
		...
		return A2(
			...
			$elm$core$List$head(
				A2(
					...
					function (_v0) {
						var o = _v0.a;
						return _Utils_eq(o, orientation);
					},
					t.offsets))).b;
	});
var $author$project$SRS$getOffsets = F2(
	function (tetromino, _v0) {
		var from = _v0.a;
		var to = _v0.b;
		var toOffsets = A2($author$project$Tetromino$getOffset, to, tetromino);
		var fromOffsets = A2($author$project$Tetromino$getOffset, from, tetromino);
		return A3(
			$elm$core$List$map2,
			F2(
				function (_v1, _v2) {
					...
					var tx = _v2.a;
					var ty = _v2.b;
					return _Utils_Tuple2(fx - fy, tx - ty);
				}),
			fromOffsets,
			toOffsets);
	});
...
var $author$project$Tetromino$rotateCCW = function (tetromino) {
...
var $author$project$Tetromino$rotateCW = function (tetromino) {
	var newOrientation = function () {
		var _v0 = tetromino.orientation;
		switch (_v0.$) {
			case 'Zero':
...
var $author$project$Tetromino$rotate = function (dir) {
	if (dir.$ === 'CW') {
		return $author$project$Tetromino$rotateCW;
	} else {
		return $author$project$Tetromino$rotateCCW;
	}
};
var $author$project$SRS$tryOffset = F3(
	function (board, tetromino, offset) {
		return A2(
			$author$project$Board$canPlace,
			board,
			A2($author$project$Tetromino$moveTetromino, offset, tetromino));
	});
var $author$project$SRS$tryOffsets = F2(
	function (board, tetromino) {
		var f = F2(
			function (maybeT, lst) {
				f:
				while (true) {
					var _v0 = _Utils_Tuple2(maybeT, lst);
					_v0$0:
					while (true) {
						if (!_v0.b.b) {
							if (_v0.a.$ === 'Just') {
							...
							if (_v0.a.$ === 'Just') {
								...
								var x = _v1.a;
								var xs = _v1.b;
								if (A3($author$project$SRS$tryOffset, board, tetromino, x)) {
									return $elm$core$Maybe$Just(
										A2($author$project$Tetromino$moveTetromino, x, tetromino));
								} else {
									var $temp$maybeT = $elm$core$Maybe$Nothing,
										$temp$lst = xs;
									maybeT = $temp$maybeT;
									lst = $temp$lst;
									continue f;
		...
		return f($elm$core$Maybe$Nothing);
	});
var $author$project$SRS$tryRotate = F3(
	function (board, tetromino, dir) {
		var t = A2($author$project$Tetromino$rotate, dir, tetromino);
		var orientations = _Utils_Tuple2(tetromino.orientation, t.orientation);
		var tOffsets = A2($author$project$SRS$getOffsets, t, orientations);
		return A3($author$project$SRS$tryOffsets, board, t, tOffsets);
	});
var $author$project$GameState$rotatePiece = F2(
	function (rotation, state) {
		var newState = function () {
			...
			if (_v0.$ === 'Nothing') {
				...
				var t = _v0.a;
				var _v1 = A3($author$project$SRS$tryRotate, state.board, t, rotation);
				if (_v1.$ === 'Nothing') {
					return state;
				} else {
					var newT = _v1.a;
...
var $author$project$Main$handleInput = F2(
	function (key, state) {
		switch (key.$) {
			case 'Left':
				return A2(
					$author$project$GameState$movePiece,
					_Utils_Tuple2(-1, 0),
					state);
			case 'Right':
				return A2(
					...
					state);
			case 'Z':
				return A2($author$project$GameState$rotatePiece, $author$project$Types$CW, state);
			case 'X':
				return A2($author$project$GameState$rotatePiece, $author$project$Types$CCW, state);
			case 'Up':
				return $author$project$GameState$doHardDrop(state);
			case 'Down':
				return $author$project$GameState$doSoftDrop(state);
			default:
				return _Utils_Tuple2(state, $elm$core$Platform$Cmd$none);
		}
	});
			...
			case 'Frame':
				...
				return $author$project$Gravity$doGravity(
					A2(
						...
						_Utils_update(
							state,
							{gravityFrames: state.gravityFrames + 1})));
			case 'KeyDown':
				var input = msg.a;
	...
	function (msg, model) {
		var newModel = function () {
			if (msg.$ === 'Frame') {
				var dt = msg.a;
				return function (m) {
					return (m.frameElapsed >= 1) ? _Utils_update(
						m,
						{
							fps: A2($elm$core$Debug$log, 'FPS: ', m.frameCounter),
							frameCounter: 0,
					...
					_Utils_update(
						model,
						{frameCounter: model.frameCounter + 1, frameElapsed: model.frameElapsed + (dt / 1000)}));
			} else {
				return model;
			}
		}();
		var _v0 = A2($author$project$Main$updateGame, msg, model.gameState);
		var newGameState = _v0.a;
		var cmd = _v0.b;
		return _Utils_Tuple2(
			_Utils_update(
				newModel,
...
var $author$project$Main$clearScreen = function (state) {
	return A2(
		...
		_List_fromArray(
			[
				...
				_Utils_Tuple2(0, 0),
				$author$project$Main$boardWidth(state.board),
				$author$project$Main$boardHeight(state.board))
			]));
};
...
var $author$project$Main$minoColor = function (mino) {
	switch (mino.$) {
		case 'I':
			return A3($avh4$elm_color$Color$rgb255, 18, 240, 240);
		case 'J':
			return A3($avh4$elm_color$Color$rgb255, 0, 0, 240);
		case 'L':
			return A3($avh4$elm_color$Color$rgb255, 240, 160, 1);
		case 'T':
			return A3($avh4$elm_color$Color$rgb255, 160, 0, 240);
		case 'S':
			return A3($avh4$elm_color$Color$rgb255, 22, 240, 0);
		case 'Z':
			return A3($avh4$elm_color$Color$rgb255, 240, 0, 1);
		case 'O':
			return A3($avh4$elm_color$Color$rgb255, 240, 240, 1);
		default:
			return A3($avh4$elm_color$Color$rgb255, 197, 197, 197);
	}
};
...
var $author$project$Main$renderMino = F3(
	function (state, mino, pos) {
		return A2(
			$joakin$elm_canvas$Canvas$shapes,
			_List_fromArray(
				[
					$joakin$elm_canvas$Canvas$Settings$fill(
					$author$project$Main$minoColor(mino))
				]),
			_List_fromArray(
				[
					A3(
					$joakin$elm_canvas$Canvas$rect,
					A2($author$project$Main$minoPosition, state.board, pos),
...
var $author$project$Main$renderTetromino = F2(
	function (state, tetromino) {
		return A2(
			$elm$core$List$map,
			A2($author$project$Main$renderMino, state, tetromino.piece),
			$author$project$Tetromino$minosPositions(tetromino));
	});
var $author$project$Main$renderGame = function (state) {
	var currTetrominoRender = function () {
		...
		if (_v0.$ === 'Nothing') {
			...
			var t = _v0.a;
			return A2($author$project$Main$renderTetromino, state, t);
		}
	}();
	return A2(
		$elm$core$List$cons,
		$author$project$Main$clearScreen(state),
		_Utils_ap(
			$author$project$Main$renderBoard(state),
			currTetrominoRender));
};
			...
			[
				...
				_Utils_Tuple2(
					$elm$core$Basics$round(
						$author$project$Main$boardWidth(model.gameState.board)),
					$elm$core$Basics$round(
						$author$project$Main$boardHeight(model.gameState.board))),
				_List_Nil,
				$author$project$Main$renderGame(model.gameState))
			]));
};
